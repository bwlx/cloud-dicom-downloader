"""Systematically test ThirdVistParm parameter names."""
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        for h_name in ["h", "hospitalCode", "HospitalCode", "HospitalID", "hospitalID", 
                       "orgCode", "OrgCode", "hosCode", "hoscode", "Hcode", "hcode"]:
            params = {**base_params, h_name: hospital_code}
            headers = make_headers(api_path)
            headers["Content-Type"] = "application/json"
            async with session.post(f"{BASE}{api_path}", json=params, headers=headers) as resp:
                text = await resp.text()
                result = decrypt_response(text)
                if "医院编号不能为空" not in result:
                    print(f"*** {h_name}: {result[:200]}")
                else:
                    pass  # Skip known error
        print("All hospital code names tried - all returned '医院编号不能为空'")
